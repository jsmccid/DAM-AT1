...
mp <- mean(test_temp$prediction)

# append your error to the output data frame, include industry and location variables
row <- cbind(ind,loc,train_error,test_error, total_error, mp)
...
december_predict_all
```
```{r}
write_csv(grouped_errors, "./appendix_files/error_ouput.csv")
write_csv(december_predict_all, "./appendix_files/december_predictions.csv")
```
...
wpr_mean <- mean(worst_perf2_rem$test_error)
...
ggplot(transactions, aes(x= trdate, y = monthly_amount)) + geom_point(size = 0.15) + facet_grid(rows = vars(transactions$industry), cols = vars(transactions$location), scales = "free")
...
```{r}
grouped_errors$pecerror <- grouped_errors$test_error/grouped_errors$mp

low_e <-grouped_errors %>%
  filter(pecerror <= 0.05)

ggplot(grouped_errors) + geom_boxplot()
```
