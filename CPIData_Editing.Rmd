...
```{r}
CPI_lm <- loess(index ~ Ndate, data = CPIData, span = 0.25, degree = 2)
...
CPI_predict_plot <- ggplot(CPIData , aes( x = Cdate, y = index)) + geom_point() + geom_line(color ="red", data = CPI_predict, aes(y = index_predict)) + geom_point(alpha = 0.2, color = "blue", data = CPI_tdates, aes(x = trdate, y = trindex_predict))

CPI_predict_plot 
```

```{r}
write_csv(CPI_tdates, "CPI_tdates.csv")
