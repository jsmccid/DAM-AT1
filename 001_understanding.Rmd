...
ggplot(transactions, aes(x= trdate, y = monthly_amount)) + geom_point(size = 0.25) + facet_grid(rows = vars(transactions$industry), cols = vars(transactions$location), scales = "free")
...
ggplot(transactions, aes(x = monthly_amount)) + geom_histogram(bins = 1000) + geom_density()

ggplot(transactions_1sd, aes(x = monthly_amount)) + geom_histogram(aes(y=..density..),bins = 100) + geom_density()
```
...
multiplot(plotlist = plotlist_loc_ind, cols = 2)
...
```
```{r}
transactions_months <- transactions %>% 
  filter(industry != 6, industry != 10) %>% 
  group_by(trdate) %>%
  summarise(mean_amount = mean(monthly_amount) , ntrans = n(), sdtrans = sd(monthly_amount))

ggplot(transactions_months, aes(x = trdate, y = mean_amount)) + geom_point() + geom_line()
```

```{r}
transactions_agg_mat <- transactions %>% 
  group_by(industry, location, trdate) %>%
  summarise(mean_amount = mean(monthly_amount) , ntrans = n(), sdtrans = sd(monthly_amount))

ggplot(transactions_agg_mat, aes(x = trdate, y = mean_amount)) + geom_line() + facet_grid(rows = vars(transactions_agg_mat$industry), cols = vars(transactions_agg_mat$location), scales = "free")
```
